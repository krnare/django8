...
class index(ListView):
    #template_name : html파일의 경로를 문자열로 저장변수
    template_name = 'blog/index.html'
    #model : 어떤 모델클래스의 객체를 리스트 업할 건지 명시하는 변수
    model = Post
    #context_object_name : 템플리셍서 사용할 객체 리스트를 저장한 변수 명
...
def searchP(request):
    ...
    #GET요청으로 온 데이터에서 name 속성이 'query'인 데이터 추출
    ...
    if t=='0':
        ...
        #filter,exclude,get 함수 사용시 : 모델클래스에 정의한 변수명 __명령어=값
